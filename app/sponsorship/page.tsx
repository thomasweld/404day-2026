...
const PREMIER_SPONSORS_QUERY = defineQuery(
  `*[_type == "homePage"][0]{
    premierSponsors { heading, subtitle, sponsors[]->{ _id, name, logo } }
  }`
);
...
export default async function SponsorshipPage() {
  const { data } = await sanityFetch({ query: PREMIER_SPONSORS_QUERY });
  const premierSponsors = data?.premierSponsors;
  return (
    <>
      {/* Hero */}
      <section className="relative py-20 overflow-hidden hero-landscape">
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-5xl sm:text-6xl md:text-7xl font-black mb-6">
            ...
            <br />
            <span className="gradient-text">404 Day</span>
          </h1>
      ...
      {/* Sponsors in Action */}
      {premierSponsors?.sponsors?.filter((s: any) => s.logo).length > 0 && (
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="section-title">{premierSponsors.heading ?? "404 Day Premier Sponsors"}</h2>
          <p className="section-subtitle mx-auto">{premierSponsors.subtitle ?? "Thank you to our 2026 Premier Sponsors!"}</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {premierSponsors.sponsors.filter((s: any) => s.logo).map((s: any) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="section-subtitle mx-auto">
              Your support keeps 404 Day free in the park. Without sponsors, this event would not be possible.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              },
              {
                ...
                desc: "Help keep 404 Day free for everyone, just like Patron does.",
              },
              {
                icon: "💌",
